...
//
// Architecture:
//   1. Authenticate with Cerberus portal (rami-levy-auth.js)
//   2. Discover PriceFull files per store (rami-levy-discovery.js)
//   3. Download + parse each store's PriceFull (reuses parseXml.js)
//   4. Write to Firebase (reuses firebaseWriter.js)
//
// Called from index.js via chain.syncModule when chain.id === 'rami-levy'.
// Returns the same result shape as syncChainMultiStore() for compatibility.
...
import { getSession }           from './rami-levy-auth.js';
...
// ── Download a single authenticated file → decompressed readable stream ──────
async function downloadAuthenticatedFile(url, cookie, label, timeoutMs = DOWNLOAD_TIMEOUT) {
  const isGz   = /\.gz(?:\?|$)/i.test(url);
  const tmpFile = join(tmpdir(), `rl-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
  ...
  if (size < 100) throw new Error(`File too small (${size} bytes) — blocked or empty`);
...
// ── Main sync entry point (called from index.js) ─────────────────────────────
export async function sync(chain, writer, config) {
  ...
  let cookie;
  try {
    cookie = await getSession({ retries: 3 });
  } catch (err) {
    result.failed    = true;
    result.failReason = `Auth failed: ${err.message}`;
    logger.fail(`${label} Authentication failed`, { error: err.message });
    return result;
  }

  // ── Step 2: Discover PriceFull files ─────────────────────────────────────
  let byStore, metrics;
  try {
    ({ byStore, metrics } = await discoverPriceFullFiles(cookie, chain.chainId, { retries: 3 }));
  } catch (err) {
    result.failed    = true;
    result.failReason = `Discovery failed: ${err.message}`;
  ...
  if (byStore.size === 0) {
    result.failed    = true;
    result.failReason = 'No PriceFull files found in Cerberus listing';
  ...
  // ── Step 3: Per-store download, parse, write ──────────────────────────────
  const db        = getDB();
  ...
  for (const [rawStoreId, fileInfo] of byStore) {
    ...
    try {
      const stream = await downloadAuthenticatedFile(
        fileInfo.url, cookie, `rami-levy-${storeId}`, config.worker?.downloadTimeout || DOWNLOAD_TIMEOUT,
      );
      ...
      // Write basic store metadata placeholder
      if (!config.dryRun) {
      ...
      // Per-store failure is isolated — does not abort other stores
  ...
  // Validation gate — same thresholds as syncChainMultiStore
  ...
  if (issues.length > 0) {
    ...
    result.failed    = true;
    result.failReason = `Validation: ${issues.join('; ')}`;
    return result;
  }
  ...
  logger.ok(`${label} Sync complete`, {
    items: result.count, stores: storeIdsSynced.length, errors: result.errors,
  });
  return result;
}
